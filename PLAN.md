...
## Step 0 — Kill-or-continue speech spike (~1 day) — **PARTIALLY DONE, verdict so far: GO**
...
- [ ] **[HUMAN]** Sit with 3 Thai Australians — specifically **lower-proficiency** ones, the acute-need
...
- [ ] **[HUMAN]** Ask each of them: **"the government offers free unlimited English classes (AMEP) —
...
- [ ] Native-speaker control: **no native speaker required** — `spike/make_controls.py` synthesizes the
      baseline with the en-AU neural voice (the same voice the product will shadow against).
- [ ] Record 5 Thai speakers (or yourself imitating the documented errors) saying flagship lines:
      *"bulk billed"*, *"referral"*, *"three times a day"*, *"turn left"*, *"the script"*.
- [ ] curl Azure Pronunciation Assessment with each. Compare against 2 native recordings.
- [ ] **The card/car contrast** (the make-or-break check): say "card" with the final /d/ dropped.
...
- [ ] Measure scoring latency on mobile data from a browser tab. Over ~1.5s, shadowing feels dead.
...
- [ ] Answer three questions in writing: (1) does the phoneme score actually separate a dropped
...
- [ ] `backend/app.py` — single-file Flask, app factory only if config actually diverges.
      `.env.example`: `DATABASE_URL`, `ANTHROPIC_API_KEY`, `AZURE_SPEECH_KEY`.
- [ ] Postgres (Docker Compose for dev). **Three tables:**
...
- [ ] Endpoints: `GET /scenarios?locale=`, `GET /scenarios/<id>` (resolve `global ∪ locale`, pack wins),
      `POST /attempts`, `GET /me/progress`.
- [ ] Seed by loading the two flagship units as JSON — hand-convert them, ~30 min, no generator needed yet.
- [ ] Store, per shadow line: the **phoneme sequence** (CMUdict, computed at build time — `en-AU` returns
      ...
      **TTS ceiling** per target phoneme (from `spike/make_controls.py`). Both are content-build outputs,
      not runtime lookups.
- [ ] **[HUMAN]** Native Thai speaker reviews the `th:` fields of the **two flagship units only** (2 files,
...
- [ ] **Day one, before anything else**: load the Azure Speech JS SDK sample on a real iPhone in Safari
...
- [ ] Plain HTML/JS page served by the same Flask app — no framework, no build step, no SPA.
- [ ] Azure Speech JS SDK in the browser for mic capture + streaming pronunciation assessment.
...
- [ ] Client POSTs the resulting scores (not the audio) to `POST /attempts`.
- [ ] **Trim leading/trailing silence with an RMS gate before sending** (~20 lines of JS). Azure bills
...
- [ ] Keep shadow-focus lines to **3–5 word chunks**. Halves billed audio, and the flagship already
      calls for short repeatable lines — free saving.
- [ ] Needs HTTPS — `getUserMedia` requires a secure context. Caddy on the droplet, or a Cloudflare tunnel.
- [ ] Roleplay endpoint: Anthropic API, system prompt built from the unit's `free_roleplay`
...
- [ ] Cost config on the roleplay call, all one-liners, all compounding: **`max_tokens` ≈ 150** (output
...
- [ ] TTS: pre-render dialogue lines to mp3 **at content-build time**, AU voice, serve as static files.
      Runtime TTS pays repeatedly for fixed content.
- [ ] Hard cap roleplay at ~12 turns/session and N sessions/day, and delete uploaded audio after 7 days
      (keep the scores). Uncapped turns are unbounded COGS; kept audio is storage plus privacy exposure
      you get nothing for.
- [ ] Email-restore escape hatch before any real user touches it: device-uuid-only means a reinstall
...
- [ ] Gamification UI: streak (minutes spoken), XP, sound meters — all read from `attempts`, no new tables.
