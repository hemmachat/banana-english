...
- `content/` — **27 units live**: all 26 Tier 1 core scenarios plus the AU GP flagship. Each has A2 + B1
  dialogues, calibrated native ceilings, en-AU audio, drafted Thai, and per-sound `scoring_method`.
  Generated by `backend/generate_unit.py` at $0.30/unit; the AU pack's `drops_from_core` is applied at
  resolution time, so an en-AU learner sees 24 (no Skytrain, songthaew or market bargaining).
...
**The content pipeline works end to end**: `generate_unit.py → render_audio.py → calibrate.py →
translate.py → seed.py`, every step idempotent and safe to re-run.

Still outstanding: Tier 2–4 and the AU pack (114 units, ~$40), and every human checkpoint — per-string
Thai review, AU fact-check, trademark, and real-user testing with lower-proficiency speakers.
